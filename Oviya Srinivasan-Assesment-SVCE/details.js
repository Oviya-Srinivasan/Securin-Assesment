function appendCVEId() {
    ...
    idcve.textContent = data.vulnerabilities[id].cve.id;
}
...
function appendDescription() {
    ...
    descriptionElement.textContent = data.vulnerabilities[id].cve.descriptions[0].value; // Assuming there's only one description for simplicity
}
...
function appendCVSSMetrics() {
    ...
    const cvssMetric = data.vulnerabilities[id].cve.metrics.cvssMetricV2[0]; // Assuming there's only one metric for simplicity
...
function appendCVSSMetricsTable() {
    ...
    data.vulnerabilities[id].cve.metrics.cvssMetricV2.forEach(cvssMetric => {
...
function appendCPETable() {
    ...
    for (let i = 0; i < 3 && i < data.vulnerabilities[id].cve.configurations[0].nodes[0].cpeMatch.length; i++) {
        const cpeMatch = data.vulnerabilities[id].cve.configurations[0].nodes[0].cpeMatch[i];
        const row = document.createElement("tr");
        row.innerHTML = `
